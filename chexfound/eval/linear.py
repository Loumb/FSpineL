...
def evaluate_linear_classifiers(
    ...
    # --- 安全构建 metric，自动调整 top-k 不超过类别数 ---
    from chexfound.eval.metrics import build_metric as original_build_metric

    def build_metric_safe(metric_type, num_classes):
        ks = [1]
        ks = [k for k in ks if k <= num_classes]
        if len(ks) == 0:
            ks = [1]
        # 对于非 Top-K metric，直接调用原函数
        try:
            return original_build_metric(metric_type, num_classes=num_classes, ks=ks)
        except TypeError:
            return original_build_metric(metric_type, num_classes=num_classes)

    metric = build_metric_safe(metric_type, num_classes=num_classes)
    # -------------------------------------------------------
...
def eval_linear(
    *,
    ...
    for data, labels in metric_logger.log_every(
        ...
        #print("img_tensor.shape:", data.shape)
...
def run_eval_linear(
    ...
    # 根据类别数限制 top-k
    # 如果类别数 < 5，就只用 top-1
    if training_num_classes <= 5:
        val_metric_type = MetricType.MEAN_ACCURACY  # top-1
        if test_metric_types is None:
            test_metric_types = [MetricType.MEAN_ACCURACY] * len(test_dataset_strs)
        else:
            # 避免 top-5
            test_metric_types = [MetricType.MEAN_ACCURACY for _ in test_metric_types]

    sampler_type = SamplerType.SHARDED_INFINITE
    # sampler_type = SamplerType.INFINITE

    n_last_blocks_list = [5]
    ...
    if img_tensor.ndim == 3:
        ...
        # 已经是 [C, H, W]，无需 permute
            img_tensor = img_tensor.unsqueeze(0)  # -> [1, C, H, W]
        elif img_tensor.shape[-1] in [1, 3]:  
            # 是 [H, W, C]，需要 permute
            img_tensor = img_tensor.permute(2, 0, 1).unsqueeze(0)  # -> [1, C, H, W]
        else:
            # 可能是 [D, H, W]，取中间切片
            img_slice = img_tensor[img_tensor.shape[0] // 2]
            img_tensor = img_slice.unsqueeze(0).unsqueeze(0)  # -> [1, 1, H, W]
    elif img_tensor.ndim == 2:
        img_tensor = img_tensor.unsqueeze(0).unsqueeze(0)  # [1, 1, H, W]

    img_tensor = img_tensor.float().cuda()
    print(">>> Final img_tensor shape:", img_tensor.shape)

    sample_output = feature_model(img_tensor)
