...
from chexfound.data.datasets import ldhNiiGzDatabase
...
class ModelWithIntermediateLayers(nn.Module):
    ...
    def forward(self, images):
        # 强制使用 2D forward，防止 3D 模式误切
        if hasattr(self, "is_3d") and self.is_3d and images.ndim == 5:
            # 只有输入是 [B, D, C, H, W] 时才走 3D
...
def evaluate_preds(
    ...
    for samples, targets, *_ in metric_logger.log_every(data_loader, 10, header):
        ...
        for k, metric in metrics.items():  # only evaluate optimal classifier
            metric_inputs = postprocessors[k](outputs, targets)
            pred = metric_inputs["preds"]
            tgt = metric_inputs["target"]
            if pred.ndim == 2 and pred.shape[1] == 2:
                pred = torch.softmax(pred, dim=1)[:, 1]
            metric.update(preds=pred, target=tgt)
...
def make_data_loaders(train_dataset, test_dataset, val_dataset=None,
    ...
    train_data_loader = make_data_loader(
        ...
        collate_fn=ldhNiiGzDatabase.collate_fn
    )
