...
class TrainConfig:
    dataset: str = MISSING
    val_dataset: str = MISSING
    val_metric_type: ClassificationMetricType = ClassificationMetricType.MEAN_ACCURACY
    batch_size: int = 128
    num_workers: int = 8
    learning_rates: Tuple[float, ...] = _DEFAULT_LR_LIST
    n_last_blocks_list: Tuple[int] = (1,)
    ...
    save_checkpoint_iterations: int | None = None
    eval_period_iterations: int | None = None
    checkpoint_retention_policy: CheckpointRetentionPolicy = CheckpointRetentionPolicy.NONE
    resume: bool = True
    classifier_fpath: Optional[str] = None
...
def setup_linear_classifiers(sample_output, n_last_blocks_list, learning_rates, batch_size, num_classes=2):
...
class Evaluator:
    ...
    def _evaluate_linear_classifiers(
        ...
        *,
        ...
        metric = build_classification_metric(self.metric_type, num_classes=2)
        postprocessors = {
...
def setup_linear_training(*, config: TrainConfig, sample_output: torch.Tensor, training_num_classes: int, checkpoint_output_dir: str):
    # linear_classifiers, optim_param_groups = setup_linear_classifiers(
    #     sample_output, config.n_last_blocks_list, config.learning_rates, config.batch_size, training_num_classes
    # )
    linear_classifiers, optim_param_groups = setup_linear_classifiers(
        sample_output,
        n_last_blocks_list=[1],  # 只取最后 1 block
        learning_rates=[1e-4],  # 单个 lr
        batch_size=32,
        num_classes=4
    )
    ...
    optimizer = torch.optim.SGD(linear_classifiers.parameters(), lr=0.001, momentum=0.9)
    ...
    start_iter = 0
    best_accuracy = -1
    if config.resume and (last_checkpoint_dir := find_latest_checkpoint(config.classifier_fpath or checkpoint_output_dir)):
        ...
        # ⚠️ 只加载匹配权重，跳过 linear 层不匹配的部分
        state_dict = checkpoint["linear_classifiers"]
        new_state_dict = {}
        for k, v in state_dict.items():
            if "linear" in k:
                print(f"⚠️ Skip loading {k} due to shape mismatch for 2-class task")
                continue
            new_state_dict[k] = v

        missing, unexpected = linear_classifiers.load_state_dict(new_state_dict, strict=False)
        print(f"✅ Linear classifier reinitialized for 2-class task. Missing: {missing}, Unexpected: {unexpected}")
        # optimizer.load_state_dict(checkpoint["optimizer"])

    criterion = nn.CrossEntropyLoss() if config.loss_type != LossType.BINARY_CROSS_ENTROPY else nn.BCEWithLogitsLoss()
    return linear_classifiers, start_iter, max_iter, criterion, optimizer, scheduler, best_accuracy
